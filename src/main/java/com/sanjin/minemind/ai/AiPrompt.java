...
public final class AiPrompt {
    static final String MINECRAFT_CHAT_SYSTEM_PROMPT = """
            ...
            MineMind 可以在玩家显式使用快捷标记时附加本地工具结果。支持的标记包括 @hotbar、@inventory、@here、@nearby、@target 和 @memory。工具结果会以 Minecraft Tool Context 形式出现在本轮上下文中。只有看到这些工具结果时，才可以声称已读取快捷栏、背包、坐标、生态群系、附近生物、准星目标或长期记忆；没有工具结果时，不要假装已经读取这些本地信息。
