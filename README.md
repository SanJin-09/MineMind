...
MineMind 是一个面向 Java 版 Minecraft 单人客户端场景的 NeoForge Mod。它允许玩家在不离开游戏的情况下，通过原版聊天栏进入 AI 对话模式，并使用玩家自己的 API Key 调用 OpenAI-compatible 文本对话模型。

当前开发进度：`v0.3 Provider 抽象`
...
MineMind 当前只做“游戏内文本 AI 对话”，不做独立聊天软件，也不读取世界状态。
...
- 多人服务器权限管理、统一计费或玩家管理
- 账号密码登录或 ChatGPT 网页账号登录
- 图片、语音、实时音频、工具调用、世界状态读取
- 长期记忆和跨游戏会话历史
- 新实体、方块、物品或 GUI 配置界面
- Gemini 原生接口及更多 Provider 适配
