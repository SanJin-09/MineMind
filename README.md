...
- [x] 模型自主工具调用，支持背包、准星目标、附近实体等白名单工具；位置和长期记忆工具需单独授权
...
| `/ai tools` | 查看模型自主工具权限 |
| `/ai tools on\|off` | 开启或关闭全部模型自主工具 |
| `/ai tools <location\|memory-read\|memory-write\|memory-delete> <on\|off>` | 设置敏感自主工具权限 |
...
默认开启，但位置和长期记忆相关自主工具默认关闭，需要玩家显式授权。模型可以自主调用以下工具：
...
```text
/ai tools
/ai tools location on
/ai tools memory-read on
/ai tools memory-write on
/ai tools memory-delete on
```
...
- 位置工具和长期记忆读取、写入、删除默认不开放给模型自主调用；玩家仍可用 `@here`、`@memory`、`@remember`、`@forget` 显式触发。
- 自主长期记忆写入需要本轮玩家输入包含明确“记住/记录/保存”等意图，且写入内容最长 `500` 字。
- 自主长期记忆删除需要本轮玩家输入包含明确“忘掉/删除/移除”等意图，删除关键词不能过短或过宽，并限制单次删除匹配数量。
