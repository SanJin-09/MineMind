...
- 支持本地 Markdown 长期记忆，玩家可显式写入、读取和遗忘记忆条目。
- 请求异步执行，不阻塞游戏主线程。
...
MineMind 支持玩家主动触发的本地文本工具。工具只会在本轮请求中附加给模型。
...
- `@remember` 支持联动 `@hotbar`、`@inventory`、`@here`、`@nearby`、`@target`，模型会根据玩家输入和已附加工具结果整理记忆。
- `@remember` 会消耗一次当前模型 API 请求；模型整理失败时不会写入文件。
- `@remember` 不会转发模型回复，只显示写入状态和最终写入内容。
- `@forget` 会把删除意图和候选记忆条目交给当前模型理解，再由后端按模型返回的条目编号删除；模型失败时不会修改文件。
...
- 只输入 `@memory` 时，MineMind 会发送默认问题：`请根据已附加的长期记忆进行简短说明。`
...
- 长期记忆以明文形式保存在本地 `config/minemind-memory.md`。
