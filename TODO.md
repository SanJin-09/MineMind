...
- [ ] 设计统一工具调用协议，优先兼容各 Provider 的原生 function/tool calling。
- [ ] 为不支持原生工具调用的 Provider 提供 JSON 工具请求 fallback。
- [ ] 增加工具调用调度器：解析模型工具请求、校验白名单、执行本地工具、回传工具结果。
- [ ] 对写文件、截图、长期记忆修改等操作增加更严格的确认或权限控制。
- [ ] 防止工具循环调用，设置单轮最大工具调用次数和超时。
- [ ] 将工具调用过程以聊天消息提示玩家，例如“已附加：背包、坐标、附近生物”。
- [ ] 为工具调用增加轻量测试和手动回归清单。
