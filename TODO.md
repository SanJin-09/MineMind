...
- [ ] 增加 `@image` 快捷标记：截取当前游戏画面并连同文本一起发送。
- [ ] 扩展 Provider 能力模型，区分 `text`、`vision`、`image-input` 等能力。
- [ ] 只有当前模型支持图片输入时才允许 `@image`，否则给出明确错误提示。
- [ ] 实现截图质量配置，例如低/中/高，控制图片大小、压缩和请求成本。
- [ ] 为 OpenAI、Gemini 等支持视觉输入的 Provider 适配不同图片请求格式。
- [ ] 截图前后提示玩家可能包含聊天、坐标、服务器名等敏感信息。
- [ ] 保持默认仍为文本模型选择，视觉模型作为显式开启能力。
...
- [ ] 支持玩家显式长期记忆写入、读取和删除。
- [ ] 不新增实体、方块、物品。
- [ ] 不支持多人服务器统一计费、玩家管理、账号密码登录、图片、语音。
