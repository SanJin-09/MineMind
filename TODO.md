...
- [ ] 在本地 Minecraft 游戏目录维护长期记忆文件，例如 `config/minemind-memory.md`。
- [ ] 增加 `@remember <内容>`：将玩家明确要求记住的内容写入长期记忆。
- [ ] 增加 `@memory`：读取长期记忆摘要或相关条目并附加到本轮请求。
- [ ] 增加记忆工具说明到前置 prompt，让模型知道可以请求读取长期记忆。
- [ ] 写入记忆前做格式化，使用日期、来源和简短条目，避免模型直接覆盖整份文件。
- [ ] 提供删除或遗忘机制，例如 `@forget <关键词>` 或后续 `/ai memory remove`。
- [ ] 限制长期记忆读取长度，必要时只读取摘要或关键词匹配结果。
